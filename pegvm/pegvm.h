...
#define PEGVM_OP_EACH(OP)                                                     \
  OP(EXIT) OP(JUMP) OP(CALL) OP(RET) OP(CONDBRANCH) OP(CHAR) OP(CHARCLASS)    \
      OP(STRING) OP(ANY) OP(PUSHo) OP(PUSHp) OP(POPo) OP(POPp) OP(GETp)       \
      OP(STOREo) OP(STOREp) OP(STOREflag) OP(STOREp_flag) OP(STOREendp)       \
      OP(NEW) OP(LEFTJOIN) OP(COMMITLINK) OP(ABORT) OP(TAG) OP(VALUE)         \
      OP(MAPPEDCHOICE) OP(SCAN) OP(IFREPEATEND) OP(DEF) OP(IS) OP(ISA)        \
      OP(BLOCKstart) OP(BLOCKend) OP(INDENT) OP(NOTCHAR) OP(NOTCHARCLASS)     \
      OP(NOTSTRING) OP(OPTIONALCHAR) OP(OPTIONALCHARCLASS) OP(OPTIONALSTRING) \
      OP(ZEROMORECHARCLASS) // OP(DTABLE)
